...
#' GlobalComparison class
#' 
#' @description 
#' The `GlobalComparison` class contains several functions for model comparison
#' and model selection of inactivation models. It should not be instanced directly. Instead,
#' it should be constructed using [compare_inactivation_fits()].
#' 
...
#' 
#' Note that all these tools use the names defined in [compare_inactivation_fits()], so 
#' we recommend passing a named list to that function.
#' 
#' @section Statistical indexes:
#' `GlobalComparison` implements two S3 methods to obtain numerical values to facilitate
...
#' - when `type = 1`, the plot compares the fitted inactivation curves against the
...
#' @describeIn GlobalComparison illustrations comparing the fitted models
#' 
#' @param x an instance of [GlobalComparison]
#' @param y ignored
#' @param ... ignored
...
#' @export
#' 
plot.GlobalComparison <- function(x, y, ...,
                                   type = 1,
                                   add_trend = TRUE) {
...
#' @describeIn GlobalComparison table of parameter estimates
#' 
#' @param object an instance of [GlobalComparison]
#' @param ... ignored
#' 
...
#' @export
#' 
coef.GlobalComparison <- function(object, ...) {
...
#' @describeIn GlobalComparison print of the model comparison
#' 
#' @param x an instance of [GlobalComparison]
#' @param ... ignored
#' 
#' @export
#' 
print.GlobalComparison <- function(x, ...) {
  
  cat("Comparison between models fitted to data using a one-step approach\n\n")
...
#' @describeIn GlobalComparison summary table for the comparison
#' 
#' @param object an instance of [GlobalComparison]
#' @param ... ignored
#' 
...
#' @export
#' 
summary.GlobalComparison <- function(object, ...) {
...
get_all_predictions <- function(model) {
  ...
  map2(model$data, model$env_conditions,
       ~ tibble(time = seq(0, max(.x$time, na.rm = TRUE), length = 100),
                logN = predict(model, 
                               times = seq(0, max(.x$time, na.rm = TRUE), length = 100),
                               env_conditions = .y
                               )
                )
       ) %>%
